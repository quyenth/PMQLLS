...
import { HuyenSaveComponent } from './../huyen-save/huyen-save.component';
...
@Component({
  selector: 'app-huyen-list',
  templateUrl: './huyen-list.component.html'
})
export class HuyenListComponent implements OnInit, OnDestroy {
  ...
  checkall = false;
  constructor(private huyenService: HuyenService, private modalService: ModalService,
  ...
  onAddNew () {
    this.modalService.openModalWithComponent(HuyenSaveComponent, { formType: FromType.INSERT, id: 0} , ModalSize.LARGE);
  }

  onEditItem(item) {
    this.modalService.openModalWithComponent(HuyenSaveComponent, { formType: FromType.UPDATE, id: item.huyenId} , ModalSize.LARGE);
  }
  ...
  onDeleteSelected () {
    ...
    if (listSelected.length === 0) {
      ...
      const dialogCloseSubscription = this.confirmationDialogService.subject.subscribe((data) => {
          dialogCloseSubscription.unsubscribe();
          if ( data === ActionType.ACCEPT) {
            this.huyenService.delectList(listSelected).subscribe((res) => {
              this.onSearch();
          });
      }
      });
