...
import { TinhSaveComponent } from './../tinh-save/tinh-save.component';
...
export class TinhListComponent implements OnInit, OnDestroy {
  ...
  onSearch (pageIndex: number = 1) {
    ...
    new SearchInfo('Tentinh', OperationType.Contains, val)
    ];
  ...
  onAddNew () {
    this.modalService.openModalWithComponent(TinhSaveComponent, { formType: FromType.INSERT, id: 0} , ModalSize.LARGE);
  }

  onEditItem(item) {
    this.modalService.openModalWithComponent(TinhSaveComponent, { formType: FromType.UPDATE, id: item.tinhId} , ModalSize.LARGE);
  }
  ...
  onDeleteItem (item) {
    ...
    let dialogCloseSubscription = this.confirmationDialogService.subject.subscribe((data) => {
        ...
        if ( data === ActionType.ACCEPT) {
          this.tinhService.delete(item).subscribe((res) => {
            this.onSearch();
        });
      }
    });
  ...
  onDeleteSelected () {
    ...
    if (listSelected.length === 0) {
      ...
      const dialogCloseSubscription = this.confirmationDialogService.subject.subscribe((data) => {
          dialogCloseSubscription.unsubscribe();
          if ( data === ActionType.ACCEPT) {
            this.tinhService.delectList(listSelected).subscribe((res) => {
              this.onSearch();
          });
      }
      });
