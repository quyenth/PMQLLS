...
import { RoleSaveComponent } from '../role-save/aspnetroles-save.component';


@Component({
  selector: 'app-role-list',
  templateUrl: './aspnetroles-list.component.html'
})
export class RolesListComponent implements OnInit, OnDestroy {
  ...
  checkall = false;
  constructor(private aspNetRolesService: RoleService, private modalService: ModalService,
  ...
  onSearch (pageIndex: number = 1) {
    ...
    if (this.orderInfo.FieldName) {
      ...
      this.aspNetRolesService.search(this.filterCondition).subscribe((res: HttpResult) => {
        this.spinner.hide();
  ...
  onAddNew () {
    this.modalService.openModalWithComponent(RoleSaveComponent, { formType: FromType.INSERT, id: 0} , ModalSize.LARGE);
  }

  onEditItem(item) {
    this.modalService.openModalWithComponent(RoleSaveComponent, { formType: FromType.UPDATE, id: item.id} , ModalSize.LARGE);
  }
  ...
  onDeleteItem (item) {
    ...
    const dialogCloseSubscription = this.confirmationDialogService.subject.subscribe((data) => {
        ...
        if ( data === ActionType.ACCEPT) {
          this.aspNetRolesService.delete(item).subscribe((res) => {
  ...
  onDeleteSelected () {
    ...
    if (listSelected.length === 0) {
      ...
      const dialogCloseSubscription = this.confirmationDialogService.subject.subscribe((data) => {
          dialogCloseSubscription.unsubscribe();
          if ( data === ActionType.ACCEPT) {
            this.aspNetRolesService.delectList(listSelected).subscribe((res) => {
            this.toastr.success('Xóa thành công!');
              this.onSearch();
          });
      }
